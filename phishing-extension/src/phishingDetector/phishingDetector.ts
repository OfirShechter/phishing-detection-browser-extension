...
import { htmlDecisionTreeUrlClassifier } from "./decisionTree.ts";

function domTypePredict(domFeatures: number[]): boolean {
    const start = performance.now();
    const isPhishing = htmlDecisionTreeUrlClassifier.predict(domFeatures);
...
export function isSiteLegitimateByUrl(url: string, signal: AbortSignal): boolean {
    ...
    const duration = performance.now() - start;
    console.log(`URL legitimacy check: ${isLegitimate} (took ${duration.toFixed(1)} ms)`);
    return isLegitimate;
}
